#!/usr/bin/env node
/* Kinetics MCP server — stdio transport. The LLM client (Claude Desktop, Cursor, …) spawns
...
try {
  init()
  const u = getUser()
  console.error(`[kinetics-mcp] serving profile ${u.name} (${u.id})`)
} catch (e) {
  console.error(`[kinetics-mcp] ${e.message}`)
