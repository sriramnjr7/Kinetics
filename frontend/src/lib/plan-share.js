...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR Kinetics — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { kinetics_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.kinetics_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an Kinetics plan file'))
  }
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with Kinetics'))} · kinetics.kinetics.example.com</footer>
</div></body></html>`
}
