# Self-hosting Kinetics

Kinetics is two small containers (a web server and an API) plus a folder of your data.
...
Kinetics signs you in with **passkeys** (WebAuthn). Browsers enforce two rules:
...
passkey prompt won't appear. To use Kinetics from your phone you need a real HTTPS hostname.
...
Put Kinetics behind something that terminates TLS for a hostname you control, then point it at
...
Kinetics only needs the browser to reach it over `https://gym.example.com`.
...
The defaults assume Kinetics is the only thing here: a service called `api` on port 3000, and nginx
...
```bash
tar czf kinetics-backup-$(date +%F).tar.gz data/
```
...
Kinetics can push two kinds of alert to your phone/desktop, even when the app isn't open:
...
you about your pushes. Kinetics sends your `ORIGIN` by default; set `VAPID_SUBJECT=mailto:you@example.com`
